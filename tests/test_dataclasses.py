...
class TestDataclasses(unittest.TestCase):
    ...
    # def test_statusA(self):
    #     """Tests statusA packet de/encoding"""

    #     test_inputMode = 0x01 # PWM Mode
    #     test_feedbackMode = 0x04 # Analog value
    #     test_valid_input = True

    #     original = icd.statusAPacket()
    ...
    def test_statusB_Positive(self):
        ...
        self.assertEqual(decoded.current, test_current)
    ...
    def test_serialNumber(self):
        ...
        original = icd.serialNumberPacket(
            hwRev=test_hw, serialNumber=test_serial, userIDA=test_IDA, userIDB=test_IDB
        )
        encoded = original.to_can_bytes()
    ...
    def test_TelemetrySettingsPacket(self):
        ...
        original = icd.TelemetrySettingsPacket(
            test_period, test_silence, test_statusA, test_statusB
        )
        encoded = original.to_can_bytes()
    ...
    def test_configPacket(self):
        ...
        # Home is optional - omit
        original = icd.configPacket(
            ...
            commandEmulation=commandEmulation,
        )
        print(f"")
        encoded = original.to_can_bytes()
        decoded = icd.configPacket.from_can_bytes(encoded)

        # Tests
        self.assertEqual(original, decoded)

        # Repeat with home
        with_home = icd.configPacket(
            enabled=test_enabled,
            channel=channel,
            reserved=reserved,
            timeout=timeout,
    ...
    def test_configPacket_raises(self):
        """Tests that configPacket __post_init__() correctly identifies invalid inputs."""
        with self.assertRaises(ValueError):
            packet = icd.configPacket(
                enabled=True,
                channel=0,
                reserved=0,
                timeout=20,  # Error!
